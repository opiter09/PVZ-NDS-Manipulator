    ...
    if (len(sys.argv) > 2):
        ...
        for i in range(numberOfFiles):
            ...
            if (size < 1000000) and (offset > 0) and ((offset + size) < len(bytE)):
                newFile = open(directory + "/" + str(offset) + "_" + str(offset + size) + "_" + str(i + 1).zfill(4) + extension, "wb")
                newFile.write(data)
                newFile.close()
            else:
                skip = skip + 1
        ...
        for i in range(skip):
            ...
            if (size < 1000000) and (offset > 0) and ((offset + size) < len(bytE)):
                newFile = open(directory + "/" + str(offset) + "_" + str(offset + size) + "_" + str(j + 1).zfill(4) + extension, "wb")
                newFile.write(data)
                newFile.close()